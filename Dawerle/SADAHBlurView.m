...
@implementation UIImage (SADAHBlurView)
...
@interface SADAHBlurScheduler : NSObject
...
@interface SADAHBlurLayer: CALayer
...
@implementation SADAHBlurScheduler

+ (instancetype)sharedInstance
{
    static SADAHBlurScheduler *sharedInstance = nil;
    if (!sharedInstance)
    {
        sharedInstance = [[SADAHBlurScheduler alloc] init];
    }
...
- (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    if (blurEnabled)
    {
        for (SADAHBlurView *view in self.views)
        {
...
- (void)addView:(SADAHBlurView *)view
{
...
- (void)removeView:(SADAHBlurView *)view
{
...
- (void)updateAsynchronously
{
    if (self.blurEnabled && !self.updating && self.updatesEnabled > 0 && [self.views count])
    {
        ...
        {
            SADAHBlurView *view = self.views[i];
...
@implementation SADAHBlurView

+ (void)setBlurEnabled:(BOOL)blurEnabled
{
    [SADAHBlurScheduler sharedInstance].blurEnabled = blurEnabled;
}

+ (void)setUpdatesEnabled
{
    [[SADAHBlurScheduler sharedInstance] setUpdatesEnabled];
}

+ (void)setUpdatesDisabled
{
    [[SADAHBlurScheduler sharedInstance] setUpdatesDisabled];
}

+ (Class)layerClass
{
    return [SADAHBlurLayer class];
}
...
- (SADAHBlurLayer *)blurLayer
{
    return (SADAHBlurLayer *)self.layer;
}

- (SADAHBlurLayer *)blurPresentationLayer
{
    SADAHBlurLayer *blurLayer = [self blurLayer];
    return (SADAHBlurLayer *)blurLayer.presentationLayer ?: blurLayer;
}
...
- (void)schedule
{
    ...
    {
        [[SADAHBlurScheduler sharedInstance] addView:self];
    }
    else
    {
        [[SADAHBlurScheduler sharedInstance] removeView:self];
    }
}
...
- (BOOL)shouldUpdate
{
    ...
    self.blurEnabled && [SADAHBlurScheduler sharedInstance].blurEnabled &&
...
- (UIImage *)snapshotOfUnderlyingView
{
    __strong SADAHBlurLayer *blurLayer = [self blurPresentationLayer];
    __strong CALayer *underlyingLayer = [self underlyingLayer];
